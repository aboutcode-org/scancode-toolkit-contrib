...
"""
Handle Godeps-like Go packages dependencies data.
...
class Dep(namedtuple('Dep', 'import_path revision comment')):
    def __new__(cls, import_path=None, revision=None, comment=None):
        self = super(Dep, cls).__new__(cls, import_path, revision, comment)
        return self


# map Godep names to our own names
NAMES = {
...
class Godep(object):
    ...
    def load(self, location):
        """
        Populate self from a location string or a file- like object containing a
        Godeps JSON.
        """
...
def parse(location):
    """
    Return a dict containing parsed Gopdes data from the file at location.
    """
